...
$(document).ready(function(){
	$('#xp').hover(
		function(){
			$(this).after('<div id="title" class="goto-xp"><div class="small-corner"></div>搴旂敤鑱旂洘</div>');
		},
		function(){
...
window.alert = $.alert = function(msg,time){
	...
	if(time>0){
		$('<div id="temp_alert_box'+randomnum+'">'+msg+'</div>').Tips({title:"鎻愮ず",width:300});
		setTimeout(function(){
		...
		$('<div id="temp_alert_box">'+msg+'</div>').dialog({title:"鎻愮ず",width:397,modal:true, buttons: [
		{
			text: "纭",
			click: function() { $(this).dialog("close");}
		}]});		
...
function _track_components_usage(){
  try{
    var filter_label = (global.filter.version === '' ? '' : '鐗堟湰 ') + (global.filter.channel === '' ? '' : '娓犻亾 ') + (global.filter.segment === '' ? '' : '鐢ㄦ埛缇�');
    var date_length = $.GetDate(global.pickedStartDay, global.pickedEndDay);
    if( window._gat != undefined){
      if( filter_label != '' ){
        _gaq.push(['_trackEvent','缁勪欢', '杩囨护鍣�', filter_label] );
      }
      _gaq.push(['_trackEvent','缁勪欢', '鏃堕棿閫夋嫨', '澶╂暟', date_length] );
    }
...
function _track_nav_bar(category, label){
  try{
    _gaq.push(['_trackEvent','瀵艰埅', category, label]);
  }catch(e){}
}
	...
	return {
		...
		init : function(target,options){
			...
			var defaults = {
				...
				panelTempl : '<div class="filterpanel" style="display:none;"><input type="text" class="input" placeholder="鎼滅储鍏ㄩ儴"/><ul class="filterlist"></ul><div class="load" style="margin:10px auto;text-align:center;display:block;"><img src="/images/pic/ajax-loader.gif"/></div><div class="submitpanel"><a href="#" class="submit">纭畾</a></div></div>'
			};
			var settings = $.extend({},defaults,options);
			settings.panelTempl = settings.panelTempl.replace('placeholder="鎼滅储鍏ㄩ儴"','placeholder="鎼滅储鍏ㄩ儴'+settings.text+'"');
...
;(function($){
	...
	$.fn.ProSelect = function(options){
		if(this.length >0){
			...
			var defaults = {
				...
				tempid : 'proTemp',
			        inputname : ['鏈€杩�60澶�','鏈€杩�30澶�','鏈€杩�7澶�','浠婂ぉ'],
			        inputval : ['-60','-30','-7','-1'],
			        custom : true,
				callback : ''
			};
	...
	function _custom($this,op){
		if(op.custom){
			var tmp = '<a class="customhref" href="#">鑷€�</a><div id="datePickerPanel" style="display:none;"><div id="datepickerStart"></div><div id="datepickerEnd"></div><input type="button" value="纭畾" class="custombtn" style="clear:both;display:block"/></div>';
...
;(function($){
	var moudle_func = {
		init : function(options) {
			...
			//榛樿type缁戝畾class.each鍑芥暟涓殑$(this)鎸囩殑鏄皟鐢╥nit鐨勫璞★紝if鍐呴儴鐨�$(this)鎸囩殑鏄�$(this).children()鍒楄〃涓殑椤�
		...
		init : function(target,options){
			...
			var defaults = {
				...
				templ : '<div class="mod singledate" id="${panelid}" style="display:none">\
						<div class="mod-header clearfix"><h2><span class="startday">2012.12.23</span> - <span class="endday">2012.12.24</span></h2><div class="option">(<span class="days"></span>澶�)</div></div>\
						<div class="mod-body">\
							<div class="">\
								<div class="tabs">\
									<ul class="clearfix">\
										<li class="lastmonth"><a href="#">涓婃湀鍚屾湡</a></li>\
										<li class="lastweek"><a href="#">涓婂懆鍚屾湡</a></li>\
									</ul>\
								</div>\
								<div class="myEndDate">\
								</div>\
								<div class="form">\
								閫夋嫨瀵规瘮鏃舵鐨勭粓鐐�<input type="button" class="submit"  value="纭畾"/>\
								</div>\
							</div>\
						</div>\
					</div>'
...
$.fn.DownList = function(options){
	...
	if(settings.is_ajax){
      ...
      if(settings.search == 'on'){
        var search = '<div id="list_search"><input type="text" class="input" placeholder="鎼滅储"></div>';
        ...
        success: function(res){
          ...
          if( res == "" ) {
            if( query == "" ) {
              ...
              $(".js-select-list").html('<li class="result_empty">鎼滅储缁撴灉涓虹┖</li>');
      ...
      formatter: function() {
        ...
        if( params['time_unit'] == 'hourly' ){
          ...
          if(date_and_time.length > 1) {
            ...
            return date_and_time[0] + " " + parseInt(hourFormat[0], 10) + "~" + (parseInt(hourFormat[0], 10)+1) +"鏃�: " + this.y;
          } else {
            return parseInt(date_and_time[0], 10) + "~" + (parseInt(date_and_time[0], 10)+1) +"鏃�: " + this.y;
          }
        }	
...
$.fn._export = function(e,options){
  ...
  getCsv(e);
	...
	function callfunction(e,data){
		...
		if(data.result == 'success'){
			inst.removeClass('export-icon-load');
			inst.after('<div class="tip-export"><div class="small-corner"></div><a class="fr close font-highlight"><b class="icon icon-close"></b></a>涓嬭浇浠诲姟宸叉彁浜�,璇峰墠寰€<a href="/apps/download_center" class="font-highlight close">鎶ヨ〃涓績</a>鏌ョ湅杩涘害&nbsp;</div>');
      setTimeout(function(){
			...
			//alert(data['msg']);
			inst.removeClass('export-icon-load');
			//inst.after('<div class="tip-export"><a class="fr close font-highlight">鐭ラ亾浜�<b class="icon icon-close"></b></a>璇峰墠寰€<a href="/apps/download_center" class="font-highlight close">鎶ヨ〃涓績</a></div>');
		}
...
var umengAdsBar = function(){
  ...
  this.showbar = function(params,adsId){
    ...
    if(params.link){
      ...
      $('#hellobar span').append('<a href="' + params.link + '" target="_blank">浜嗚В鏇村</a>');
    }else{
      $('#hellobar span').text(params.content);
    }
    if(params.id != adsId){
